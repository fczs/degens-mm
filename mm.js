...
let myAddress = config.privateKey
  ? new ethers.Wallet(config.privateKey).address.toLowerCase()
  : undefined;
...
function doUpdates() {
  ...
  DegensUtils.withMutexGate("doUpdates", async () => {
    ...
    for (let event of Object.values(dc.getEvents())) {
      if (now - config.orderRefreshTimeSeconds < (lastEventUpdate[event.eventId] || 0))
        continue;
      ...
      for (let marketType of Object.keys(marketLookup)) {
        ...
        marketIds.sort((a, b) =>
          Math.sign(
            Math.abs(0.5 - marketPrices[a].prob) - Math.abs(0.5 - marketPrices[b].prob)
          )
        );
...
function genericCompare(val, test) {
  ...
  else if (typeof test === "object" && Array.prototype.isPrototypeOf(test))
    return !!test.find((item) => item === val);
...
function computeMyPrices(event, marketLookup, tokenSym) {
  ...
  let currPositions;
  if (dc.data.positions[myAddress].events[event.eventId])
    currPositions = dc.data.positions[myAddress].events[event.eventId].matches;
  ...
  if (marketLookup["1x2_1"] && marketLookup["1x2_x"] && marketLookup["1x2_2"]) {
    for (let strat of config.strategies) {
      ...
      let priceSet = [
        providerPrices[marketLookup["1x2_1"]],
        providerPrices[marketLookup["1x2_x"]],
        providerPrices[marketLookup["1x2_2"]],
      ].filter((p) => !!p);
  ...
  */
  ...
  for (let marketId of Object.keys(output)) {
    let p = output[marketId];
    let position =
      currPositions && currPositions[marketId] && currPositions[marketId].tokens[tokenSym];
    ...
    if (position) {
      ...
      let atRisk = myPos
        .abs()
        .mul(myPos.gt(0) ? position.avgPrice : DegensUtils.MAX_PRICE - position.avgPrice)
        .div(DegensUtils.MAX_PRICE);
    ...
    if (
      existingOrders.asks.length &&
      existingOrders.bids.length &&
      existingOrders.asks[0].price < existingOrders.bids[0].price
    ) {
    ...
    if (existingOrders.asks.length && buyPrice > existingOrders.asks[0].price) {
      let numMyOrders = db
        .prepare("SELECT COUNT(*) FROM LiveOrder WHERE degensMatchId = ?")
        .pluck()
        .get(marketId);
...
function constructOrder(marketId, direction, amount, price, expiry) {
  ...
  let existingOrders = db
    .prepare("SELECT * FROM LiveOrder WHERE degensMatchId = ? AND direction = ?")
    .all(marketId, direction);
  ...
  if (existingOrder) {
    if (
      ...
      now - existingOrder.timestamp < pollIntervalSeconds * 2
    )
      return undefined; // don't refresh orders too often

    let existingOrderAmount = ethers.utils.bigNumberify(existingOrder.amount);
    let amountRemaining = existingOrderAmount.sub(
      dc.data.orderFills[myAddress].recs[existingOrder.fillHash] || 0
    );
  ...
  if (existingOrder) {
    ...
    db.prepare(
      "UPDATE LiveOrder SET price = ?, timestamp = ?, expiry = ? WHERE orderId = ?"
    ).run(price, now, expiry, existingOrder.orderId);
  } else {
    db.prepare(
        `INSERT INTO LiveOrder (degensMatchId, direction, price, timestamp, expiry, maker, token, amount, orderGroup, fillHash)
                    VALUES (?,?,?,?,?,?,?,?,?,?)`
    ).run(
      marketId,
      direction,
